...
import type { StudentPortalData, StudentProfile } from "./types";
...
function serializeProfile(
  profile: Omit<StudentProfile, "createdAt" | "emailVerifiedAt" | "lastLoginAt"> & {
    createdAt: Date;
...
export function emptyStudentPortal(
...
export async function getStudentPortalData(
  ...
  try {
    const [profile, courses, enrollmentRequests, quizAttempts, sessions, counsellingCount] =
      await Promise.all([
        db.user.findUnique({
          where: { id: userId },
    ...
    return {
      ...
      courses,
      enrollmentRequests: enrollmentRequests.flatMap((request) =>
        request.course
          ? [
              {
                ...request,
      ...
      quizAttempts: quizAttempts.map((attempt) => ({
        ...
        courseTitle: attempt.quiz?.lesson?.module?.course?.title ?? "Course",
        score: attempt.score,
        total: attempt.total,
    ...
    console.error("Student portal data failed:", error);
    const profile = await db.user.findUnique({
      where: { id: userId },
      select: studentProfileSelect,
    });
    if (!profile || profile.role !== "STUDENT") return null;
    return emptyStudentPortal(serializeProfile(profile), currentSessionId);
  }
}
