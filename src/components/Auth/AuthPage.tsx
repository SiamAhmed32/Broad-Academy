...
export default function AuthPage({
  ...
  const [message, setMessage] = useState(oauthMessage(oauthError));
      ...
      >
        <section className="flex items-center justify-center px-5 py-8 sm:px-10 lg:px-14 xl:px-20">
          <div className="w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} noValidate className="mt-7 space-y-4">
              ...
              {message && (
                ...
                >
                  <p>{message}</p>
                  {oauthDebug ? (
                    <p className="mt-2 break-words text-xs text-red-500">{oauthDebug}</p>
                  ) : null}
                </motion.div>
              )}
            ...
            <div className="mt-5 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
            ...
            <a
              ...
              className="mt-5 flex h-12 w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-navy shadow-sm transition hover:border-slate-300 hover:bg-slate-50"
            >
              <GoogleMark />
...
function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true">
      ...
      <path
        fill="#FBBC05"
        d="M5.84 14.09A6.97 6.97 0 0 1 4.5 12c0-.77.13-1.53.34-2.25V7.91H2.18A11.97 11.97 0 0 0 0 12c0 1.94.46 3.77 1.18 5.35l3.66-2.84Z"
      />
      <path
        fill="#EA4335"
        d="M12 4.75c1.62 0 3.07.56 4.21 1.64l3.15-3.15C17.45 1.09 14.97 0 12 0 7.7 0 3.99 2.47 2.18 6.91l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53Z"
      />
    </svg>
  );
}

function oauthMessage(error?: string) {
  switch (error) {
    case "google_denied":
      return "Google sign-in was cancelled. You can try again or use email.";
    case "google_unverified":
      return "That Google account email is not verified yet.";
